...
Survtabler way takes 88% of less lines of code to write and the code is way easier to read. The advantage of using survtabler is way more obvious when number of model combinations increases to dozens, hundreds oreven thousands. Furthermore, survtabler function-based scripts are easier to work with when analyses often need to be refined later. It would be much nicer to e.g. tweak the covariates here in one line than in possibly dozens.
